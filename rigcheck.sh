...
if [ "$overheat" -eq 1 ]
	then
		/usr/bin/curl -m 5 -s -X POST --output /dev/null https://api.telegram.org/bot${apikey}/sendMessage -d text="${rigname} overheated! Overheated GPU: ${overheatedgpu} Using clear-thermals." -d chat_id=${chatid}
		/opt/ethos/bin/clear-thermals
		exit 1
	fi
